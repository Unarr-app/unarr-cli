...
- **desktop**: record the host going down in crash reports
...
- **docker**: build with Go 1.26 to match go.mod
...
- **desktop**: open support reports with the daemon run and log freshness

### Fixed

- **daemon**: treat a boot log held by the task's shim as a daemon already owned
- **daemon**: fall back to a detached daemon when the Windows task will not run
...
- **subs**: serve raw .ass + embedded fonts, and filter drawing cues from WebVTT
...
- **windows**: stop console window flashing at boot via wscript VBS launcher
...
- **windows**: address code-review findings + fix task-XML encoding regression
- **windows**: stop console windows flashing on spawned children + harden autostart
...
- **agent**: report install_type on register (cli/desktop/docker)
- **desktop**: play served sting+feature .m3u via playlist= deep-link param
...
- **desktop**: accept sub= in the play deeplink and pass it to the player
...
- **stream**: unbreak QSV transcodes that produced zero bytes
...
- **doctor**: add --fix self-repair for common misconfigurations
...
- **library**: gofmt integrity + types doc comments
...
- **config**: default to unarr.app primary, TorrentClaw as failover
- **release**: auto-publish Homebrew formula in CI; drop dead Forgejo prune
- **stream**: allow Google Cast receiver + wildcard CORS

### Changed

- **release**: migrate Homebrew brews → homebrew_casks

### Documentation

- **dockerhub**: rewrite for unarr brand + GitHub-native releases

### Fixed

- **ci**: check out the dispatched tag in the release workflow
- **ship**: run buildx on a host-network builder for CDN egress

### Other

- **brew**: prep goreleaser tap block for Unarr-app/homebrew-tap
- **release**: 1.3.1-beta
...
- **release**: mirrors are independent, not byte-identical
