...
func TestBuildStateIcons(t *testing.T) {
	...
	for _, st := range []trayState{stateRunning, statePaused, stateStopped, stateCrashed} {
		data := icons[st]
		if len(data) == 0 {
	...
	// Non-running variants must actually differ from the base logo.
	for _, st := range []trayState{statePaused, stateStopped, stateCrashed} {
