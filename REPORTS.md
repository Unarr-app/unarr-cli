...
Upload is available in every release build and the backend endpoint is always
active. A network connection exposes the source IP to the services carrying it,
and normal infrastructure may log it, but the IP is not part of the report
payload or support email. The server capability check occurs only after upload
consent. No legacy support or email fallback is used when private delivery is
unavailable.
